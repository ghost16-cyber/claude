  ...
  return (
    <Section title="">
      ...
      <div id="users-page" className="flex justify-between items-start -mt-2 mb-2 flex justify-between items-center gap-3 ">
        <button onClick={onBack} className="px-3 py-1.5 rounded-lg border border-zinc-700 bg-zinc-900 text-sm hover:bg-zinc-800  self-start">
          ← Indietro
        </button>
        <h2 className="flex items-center justify-between gap-2 text-lg font-semibold">Impostazioni · Utenti</h2>
      </div>
                  ...
                  <tr key={u.id} className="border-t border-zinc-800">
                    ...
                    <td className="py-2 ">
                      {canEdit ? (<div className="inline-flex flex-col items-end gap-1">
                          <button className="text-xs underline   self-start" onClick={()=>setEditing({...u} as any)}>Modifica</button>
                          <button className="text-xs underline text-red-400  self-start" onClick={()=>del(u)}>Elimina</button>
        ...
        <div className="border border-zinc-800 rounded-2xl p-4">
          ...
          <form onSubmit={createUser} className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 flex justify-end">
              <button disabled={!canEdit} className="rounded-lg bg-zinc-100 text-zinc-900 px-4 py-2 text-sm font-medium hover:bg-white disabled:opacity-50  self-start">Crea</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center">
          <form onSubmit={saveEdit} className="w-[520px] rounded-2xl border border-zinc-700 bg-zinc-900 p-5">
            ...
            <div className="mt-4 flex justify-between gap-2">
              <button type="button" className="px-3 py-2 rounded-lg border border-zinc-700  self-start" onClick={()=>setEditing(null)}>Annulla</button>
              <button disabled={!canEdit} className="px-3 py-2 rounded-lg bg-zinc-100 text-zinc-900 disabled:opacity-50  self-start">Salva</button>
            </div>
          </form>
        </div>
      )}
